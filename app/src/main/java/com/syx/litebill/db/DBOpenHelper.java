...
public class DBOpenHelper extends SQLiteOpenHelper {
    ...
    public DBOpenHelper(@Nullable Context context) {
        super(context,"liteBill.db",null,1);
    }
...
//     创建数据库，仅项目第一次运行时会被调用
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        String sql = "create table typetb(id integer primary key autoincrement, typename varchar(10)," +
                "imageId integer,selectedImageId integer, kind integer)";
        db.execSQL(sql);
        insertType(db);
        // 创建记录表
    ...
    private void insertType(SQLiteDatabase db) {
        //向typetb表中插入元素
        String sql = "insert into typetb(typename,imageId,selectedImageId,kind) values(?,?,?,?)";
